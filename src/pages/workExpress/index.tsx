...
const WorkExpressPage: NextPage<{ posts: [TWorkExpress] }> = ({ posts }): ReactElement => {
  return (
    <Layout title="Work Express" page="workExpress">
      <Content page="workExpress">
        <section>
          ...
          <p className="mt-6">これまでキャリアを最新のものから時系列に載せています。</p>
        </section>
        <section className={`mt-6 ${workExpressStyles.listWrap}`}>
          <ol className={`border-b border-dashed border-black border-opacity-50 relative lg:ml-20 ${workExpressStyles.listBlock}`}>
            {posts.map((post: TWorkExpress) => (
              <li key={post.id} className="pt-6 pb-4">
                <h2 className={`font-bold relative ${workExpressStyles.listHeading}`}>
                  {post.project}
                  <span className="font-normal text-xs text-gray-400">1年間</span>
                  <span className={`mt-1 font-normal block text-xs text-gray-400 ${workExpressStyles.listDurationContainer}`}>
                    <span className={`relative lg:pb-2 ${workExpressStyles.listDuration}`}>{post.start_date}</span>
