...
const fetcher = async (url: string, query: { [service: string]: string }): Promise<TBlog[]> => {
  if (!!query) {
    url += "?";
...
const useBlogDataSWR = (fallbackData: TBlog[], query: { [service: string]: string }): SWRResponse<any, any> => {
  return useSWR([apiBlogDomain, query], fetcher, { fallbackData });
};

const BlogPage: NextPage<{ fallbackData: TBlog[] }> = ({ fallbackData }): ReactElement => {
  const pageType: string = "blog";

  const [state, setState] = useState({ type: "all" });
  const query = { service: state.type };
  const { data } = useBlogDataSWR(fallbackData, query);
  ...
  useEffect(() => {
    const buttonLists: NodeListOf<Element> = document.querySelectorAll(".js-button-list");
        ...
        <section>
          ...
          <ul className="flex justify-center mt-4 mb-2 text-black lg:justify-start">
            <li className="mr-2 js-button-list">
              <button data-type="twitter" onClick={handleChangeBlogType} className={`${blogStyles.button} text-center`}>
                Twitter
              </button>
            </li>
            <li className="mr-2 js-button-list">
              <button data-type="zenn" onClick={handleChangeBlogType} className={`${blogStyles.button} text-center`}>
                Zenn
              </button>
            </li>
            <li className="mr-2 js-button-list">
              <button data-type="all" onClick={handleChangeBlogType} className={`${blogStyles.button} text-center`}>
                All
              </button>
            </li>
          </ul>
          <ul className="lg:flex lg:flex-wrap lg:gap-8 lg:justify-center">
            {!!data ? (
              data.map((blog: TBlog, i: string) => {
                return (
                  <li key={i} className={`w-full px-4 pt-4 pb-6 mt-4  ${blogStyles.articleBlock}`}>
                    <h2 className={`mb-5 ${blogStyles.articleHeading}`}>
                      <span className={`block text-center mx-auto leading-6 rounded-xl w-24 h-6 text-white text-base ${blogStyles.twitter}`}>{blog.service}</span>
                      <span className="block text-center text-base">{blog.date_created}</span>
                      <span className={`block mt-2 mx-auto w-24 h-px ${blogStyles.twitter}`}></span>
                    </h2>
                    <p className="px-4 mb-5">{omittedText(blog.text, 100)}</p>
                    <Link href={blog.link} passHref>
                      <a className={`block text-center mx-auto rounded ${blogStyles.articleReadmore}`} target="_blank" rel="noopener noreferrer">
                        READ MORE
                      </a>
                    </Link>
                  </li>
...
export const getStaticProps: GetStaticProps = async (): Promise<{
  ...
  const blogData = await getBlogsData("all");
